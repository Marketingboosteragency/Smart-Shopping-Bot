# app.py (versión 14.1 - Motor de Búsqueda Híbrido - Completo)

# ==============================================================================
# SMART SHOPPING BOT - APLICACIÓN COMPLETA CON FIREBASE
# Versión: 14.1 (Hybrid Search Engine - Full Code Verified)
# Novedades:
# - Código completo y verificado, listo para copiar y pegar.
# - Motor de búsqueda dual que consulta Google Orgánico y Google Shopping en paralelo.
# - Garantiza la obtención de resultados de las principales tiendas gracias a Google Shopping.
# ==============================================================================
...
def _verify_is_product_page(query: str, page_title: str, page_content: str, category: str) -> bool:
    ...
    if category == "industrial_parts":
        prompt_template = (f"You are a B2B product verification analyst. The user is searching for an industrial part: '{query}'. I found a webpage titled '{page_title}'. Is this page from a specialized distributor, manufacturer, or B2B supplier offering the main product for sale? Exclude general marketplaces like Amazon/eBay, informational articles, and forums. Answer with only the word YES or NO.")
    else:
        prompt_template = (f"You are a product verification analyst. The user is searching for a tech gadget: '{query}'. I found a webpage titled '{page_title}'. Is this page offering the main product itself for sale, and not just an accessory, review, or news article? Answer with only the word YES or NO.")
    try:
        model = genai.GenerativeModel('gemini-1.5-flash-latest')
...
class SmartShoppingBot:
    ...
    def get_query_from_image(self, image_content: bytes) -> Optional[str]:
        ...
        try:
            ...
            response = self.vision_client.annotate_image(request={'image': image_for_api, 'features': features})
