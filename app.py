# app.py (versión 12.0 - Cerebro de Búsqueda Adaptativo)

# ==============================================================================
# SMART SHOPPING BOT - APLICACIÓN COMPLETA CON FIREBASE
# Versión: 12.0 (Adaptive Search Brain)
# Novedades:
# - El bot ahora detecta si el producto es "industrial" o "tecnología de consumo".
# - Utiliza prompts de IA y estrategias de búsqueda diferentes para cada categoría.
# - Se especializa en ambos nichos para obtener resultados de máxima calidad.
# ==============================================================================
...
def _get_product_category(query: str) -> str:
    """Usa IA para clasificar la búsqueda como industrial o de consumo."""
...
def _verify_is_product_page(query: str, page_title: str, page_content: str, category: str) -> bool:
    """Verifica si la página es relevante, usando un prompt adaptado a la categoría."""
...
class SmartShoppingBot:
    ...
    def search_with_ai_verification(self, query: str, category: str) -> List[ProductResult]:
        if category == "industrial_parts":
            ...
            blacklist = ['amazon.com', 'walmart.com', 'ebay.com', 'alibaba.com', 'aliexpress.com', 'etsy.com']
        else:
            search_query = query
            blacklist = []

        print(f"--- Búsqueda final en SerpApi: '{search_query}' ---")
        params = {"q": search_query, "engine": "google", "tbm": "shop" if category == "consumer_tech" else None, "location": "United States", "gl": "us", "hl": "en", "num": "25", "api_key": self.serpapi_key}
        ...
        try:
            ...
            # Usar 'shopping_results' para tech y 'organic_results' para industrial
