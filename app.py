...
class SmartShoppingBot:
    ...
    def get_query_from_image_vision_api(self, image_content: bytes) -> Optional[str]:
        ...
        try:
            ...
            if response.web_entities:
                for entity in response.web_entities[:5]:
                    if entity.score > 0.6: all_words.extend(entity.description.lower().split())
    ...
    def _combine_text_and_image_query(self, text_query: str, image_query: str) -> str:
        ...
        try:
            model = genai.GenerativeModel('gemini-1.5-flash-latest')
            prompt = f"A user is searching for a product. Text: '{text_query}'. Image keywords: '{image_query}'. Combine these into a single, effective search query. Respond only with the final search query."
            response = model.generate_content(prompt)
    ...
    def search_product(self, query: str = None, image_content: bytes = None) -> Tuple[List[ProductResult], List[str]]:
        ...
        if text_query and image_query:
            print(f"🧠 Combinando texto '{text_query}' e imagen '{image_query}'..."); final_query = self._combine_text_and_image_query(text_query, image_query)
